...
function isValidRenderer(_a, reportError) {
    ...
    if (rendererPackageName !== "react-dom" ||
        ...
        reportError &&
            reportError("Unsupported React renderer (only react-dom v".concat(exports.MIN_SUPPORTED_VERSION, "+ is supported). Renderer: ").concat(rendererPackageName || "unknown", ", Version: ").concat(version || "unknown"));
        return false;
    }
    if (bundleType !== exports.BUNDLE_TYPE_DEV) {
        reportError &&
            reportError("Unsupported React renderer, only bundle type ".concat(exports.BUNDLE_TYPE_DEV, " (development) is supported but ").concat(bundleType, " (").concat(bundleType === exports.BUNDLE_TYPE_PROD ? "production" : "unknown", ") is found"));
        return false;
    }
    return true;
}
