...
import { InstallByAnything } from "./InstallByAnything";
import {
  BlockHeadline,
  InlineCode,
  InstallContainer,
...
import { AlternativelyInstallExtension } from "./AlternativelyInstallExtension";

type InstallJsxFrameworkProps = {
  name: string;
  viteImport: string;
  vitePlugin: string;
};

export function InstallJsxFramework({
  name,
  viteImport,
  vitePlugin,
}: InstallJsxFrameworkProps) {
  ...
  return (
    <>
      ...
      <section className="overflow-hidden text-gray-600 body-font dark:text-gray-400 dark:bg-gray-900">
        <InstallContainer>
          <BlockHeadline>Install Locator for {name}</BlockHeadline>
          <p className="text-center">Install Locator on {name} codebase.</p>
          <StepsBody>
